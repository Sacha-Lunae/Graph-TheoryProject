...
def returnEarliest(n):
    #That's exactly the same thing as above, but I need to make it a "return" for the traces - Sacha
    rankDico = find_rank(add_column_number(del_omega(getAdjacencyMatrix(n))), rank=0, ranklist={})  # dictionary containing vertex and their rank
    listRank = []
    listVertex = []
    for value in rankDico.values():
    ...
    return("Ranks             |" + str(listRank) + "| \nVertex            |" +str(listVertex)+
            "| \nPredecessors      |" + str(listPred) + "| \nInitial duration  |"+ str(listEarlPredINIT)+
            "| \nEarliest dates    |"+ str(listEarlPredFINAL)+ "|" )